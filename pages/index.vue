...
<script>
    ...
    function getGazers(context) {
        ...
        const url = 'https://api.github.com/repos/uikit/uikit';
        const p = process.server ? got(url, {json:true}) : ajax(url, {responseType: 'json'});
        return p.then((res) => {

            const response = res.response || res.body;
